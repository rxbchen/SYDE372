...
[X1,X2] = meshgrid(1:step:size);
win_a = mvnpdf([X1(:) X2(:)], mu, Sigma);
win_a = reshape(win_a,length(X2),length(X1));
...
Parz_abc = combined_classifier_parzen(Parz_ab, Parz_bc, Parz_ca, x_pa, y_pa);
%figure;
contourf(X2_pt, Y2_pt, Parz_abc);
hold on;
class_c = scatter(at(:, 1), at(:, 2), 'rx');
class_d = scatter(bt(:, 1), bt(:, 2), 'mo');
class_e = scatter(ct(:, 1), ct(:, 2), 'w+');
